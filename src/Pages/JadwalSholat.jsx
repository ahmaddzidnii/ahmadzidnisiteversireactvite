...
const JadwalSholat = () => {
  ...
  const getKotas = async () => { // Membuat fungsi async "getKotas" untuk mendapatkan data kota.
    try {
      const response = await axios.get(`${import.meta.env.VITE_JADWAL_SHOLAT_URL}/kota/semua`); // Melakukan permintaan HTTP GET menggunakan axios untuk mendapatkan data kota.
    ...
    } catch (err) { // Menangkap kesalahan jika permintaan gagal.
  ...
  useEffect(() => { // Menggunakan useEffect untuk melakukan efek samping saat komponen dipasang.
  ...
  if (loading) { // Jika "loading" bernilai true, tampilkan indikator loading.
    return (
  ...
  } else { // Jika "loading" bernilai false, tampilkan daftar kota.
    return (
      <div className="bg-sky-50">
        <div className="container space-navbar">
          ...
          <div className="row">
            {kotas.filter(f => {
              return search === "" ? true : f.lokasi.toLowerCase().includes(search.toLowerCase());
            }).map((k , index) => (
              <div className="col-md-4" key={index}>
                <Link to={`/Jadwalsholat/listkota/${k.id}`} className="card-hover card my-2 text-decoration-none border-black">
